...
function SummaryGroupHeader({
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const ts = await getEventStartTime(db, first.endTime);
        if (!cancelled) setStart(ts !== null ? new Date(ts) : null);
      } catch (err) {
        console.error('Failed to compute start time for summary group', err);
      ...
      <div className="flex-1 flex flex-row items-center pl-4 pr-2 py-2 gap-4 overflow-x-hidden whitespace-nowrap">
        ...
        <div className="grow overflow-x-hidden">
          <span className="text-muted-foreground text-xs mr-3">
            {formatTime((start ?? new Date(first.endTime)).getTime())}–{formatTime(last.endTime)}
          </span>
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('Failed to compute start time for summary row', err);
        if (!cancelled) setStart(null);
      }
    })();
      ...
      <div className="flex-1 flex flex-row items-center pl-4 pr-2 py-2 gap-3">
        ...
        <div className="grow">
          <span className="inline-block text-muted-foreground text-xs mr-3">
            {formatTime((start ?? new Date(event.endTime)).getTime())}–{formatTime(event.endTime)}
          </span>
